...
% 			standAloneRun([],subjParams.protocol,subjParams.id);
            makeNew = subjParams.subjectDetails.removeDB;
            rx=getOrMakeDefaultRatrix(makeNew,subjParams.startEyelink);   
            auth='ratrix';
            ...
            if needToAddSubject
                sub = subject(subjParams.id, subjParams.subjectDetails.species, subjParams.subjectDetails.strain, subjParams.subjectDetails.sex, ...
                    '05/10/2005', '01/01/2006', 'unknown', 'Bred In-house',subjParams.subjectDetails.geneticBackground,subjParams.subjectDetails.geneticModification);
                rx=addSubject(rx,sub,auth);
            end
            
            su=str2func(subjParams.protocol); %weird, str2func does not check for existence!
            otherParams.stepNum = subjParams.trainingStepNum;
            otherParams.maxWidth = subjParams.rigParams.maxWidth;
            otherParams.maxHeight = subjParams.rigParams.maxHeight;
            otherParams.distance = subjParams.rigParams.distance;
            otherParams.height = subjParams.rigParams.height;
            otherParams.angle = subjParams.rigParams.angle;
            
            rx=su(rx,{subjParams.id},otherParams);
